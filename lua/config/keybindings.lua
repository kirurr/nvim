...
map("n", "<C-b>d", ":bd<CR>", assign_opts("Delete current buffer", nil))
map("n", "<C-b>p", ":bp<CR>", assign_opts("Previous buffer", nil))
map("n", "<C-b>n", ":bn<CR>", assign_opts("Next buffer", nil))
...
local on_attach_keybindings = function(bufnr)
	...
	map("n", "<C-k>", vim.lsp.buf.signature_help, assign_opts("Signature help", bufnr))
